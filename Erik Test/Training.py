...
def MakeBrainFile():
    ...
    for sentence in training_data:
        ...
        w = list(trigrams(sentence['sentence']))
        tempWords = []
        
        
        for x in w:
            tempWords.append(x[0] + x[1] + x[2])
        print(tempWords)
        words.extend(tempWords)
        # add to documents in our corpus
        documents.append((tempWords, sentence['class']))
        
        # add to our classes list
        if sentence['class'] not in classes:
            classes.append(sentence['class'])
    ...
    print (len(tempWords), "unique stemmed words")
